...
class Shimmer3_sensor(object):
    ...
    def __init__(self, Serial_Port=None, Conf=0):
        ...
        self.Allowed_baud_rates = {'115200': 0, '1200': 1, '2400': 2, '4800': 3, '9600': 4, '19200': 5, '38400': 6,
                                   '57600': 7, '230400': 8, '460800': 9, '921600': 10} 
                                   # The aviable Serial Baud rates in the Shimmer3 module and the byte number for configurating it

        #Serial Shimmer Values
        self.Allowed_accelerometer_ranges = {'2g': 0, '4g': 1, '8g': 2, '16g': 3} # The aviable accelerometer ranges    
...
# Set methods -- Set methods -- Set methods -- Set methods -- Set methods
    ...
    def set_sensors_EMG(self, show=False):

        response = self.write_port([ Shimmer3_Serial_Commands['SET_SENSORS_COMMAND'], 0xF0, 0x00, 0x00]) # Sending the byte command to configure sensors and disnable all sensors with 0x00 bytes
        if response:
            if show:
                print('The sensors and the EMG channel 1 were successfully enabled')
            return True
        else:
            print('set_sensors method: It was NOT possible set the sensors')
            return False

    def set_EMG(self, show=False):

        response = self.write_port([ Shimmer3_Serial_Commands['SET_SENSORS_COMMAND'], 0x10, 0x00, 0x00]) # Sending the byte command to configure sensors and disnable all sensors with 0x00 bytes
        if response:
            if show:
                print('The sensors EMG channel 1 was successfully enabled')
            return True
        else:
            print('set_sensors method: It was NOT possible set the sensors')
            return False
...
if __name__ == "__main__":
    textFile = open('Prueba_EMG.txt','w')
    
    Sesnor_Data = []
    sensor = Shimmer3_sensor('COM4');
    sensor.open_port(show=True)
    #sensor.set_serial_baud_rate(baud='921600',show=True)

    sensor.disenable_sensors(show=True)

    sensor.set_sample_frequency(512,show=True)

    sensor.set_EMG(show=True)
    ...
    try:
        while 1:
            ...
            Status = struct.unpack('B', data[5])
            c1ch1 = struct.unpack('>i', (data[5:8]  + '\0'))[0] >> 8
            #c1ch2 = struct.unpack('>i', (data[8:data_size]  + '\0'))[0] >> 8

            timestamp = timestamp0 + timestamp1*256 + timestamp2*65536
            #EMG_CH1 = EMG_CH1_H + EMG_CH1_M*256 + EMG_CH1_L*65536
            #Sesnor_Data.append(timestamp)
            print ( "0x%02x,%5d,\t%5d" % (packettype[0], timestamp, c1ch1))
            textFile.write(str(timestamp) + ',' + str(c1ch1) + '\n')
